...
var utils = module.exports = {
  ...
  extractProgress: function(command, stderr, duration) {
    var lines;
    var lastline;
    if (command.options.outputLineLimit === 1){
        lastline = stderr //use the line without splitting
        lines = []
    }else{
        lines = stderr.split(nlRegexp);
    ...
    if (progress) {
      ...
      if (lines.length >= command.options.outputLineLimit && command.options.outputLineLimit > 1){
          //flag for processor
